...
export default function AuthPage() {
  ...
  const sendOtp = async () => {
    ...
    const data = await parseResponseData(response);
    if (!response.ok) {
      const tips = Array.isArray(data.troubleshooting) ? ` Tips: ${(data.troubleshooting as string[]).join(' | ')}` : '';
      const code = data.snsErrorCode ? ` [${String(data.snsErrorCode)}]` : '';
      const raw = data.snsError ? ` (${String(data.snsError)})` : '';
      return setMessage(`${(data.error as string) || 'Unable to send OTP.'}${code}${raw}${tips}`);
    }
    setMessage(String(data.message || 'OTP sent to your phone.'));
  };
