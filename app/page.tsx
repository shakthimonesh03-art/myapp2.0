...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const nextRole = (user?.role || 'guest') as 'guest' | 'customer' | 'admin';
    setRole(nextRole);
    ...
    <section className="modern-page">
      ...
      <main className="modern-main">
        <header className="modern-header">
          <h1>Welcome back!</h1>
          <p>
            Here is what&apos;s happening in <span>{selectedLocation || 'your area'}</span>
          </p>
        </header>
          ...
          {filtered.map((event, index) => (
            <article key={event.id} className="modern-card" style={{ animationDelay: `${index * 60}ms` }}>
              <div className="event-visual">
                <img src={event.imageUrl} alt={`${event.title} poster`} loading="lazy" referrerPolicy="no-referrer" />
              </div>
              <div className="icon-chip">{index + 1}</div>
              <h3>{event.title}</h3>
              <p>
                {event.city} • {formatDateTime(event.datetime)}
              </p>
              <p>{event.venue}</p>
