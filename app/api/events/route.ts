...
export async function POST(req: Request) {
  ...
  if (body.action === 'create') {
    ...
    persistServiceState('events.create', { eventId: event.id });
    await publishKafkaEvent('event.created', { eventId: event.id, payload: event });
  ...
  if (body.action === 'update') {
    ...
    persistServiceState('events.update', { eventId: event.id });
    await publishKafkaEvent('event.updated', { eventId: event.id, payload: body.updates || {} });
    return json({ event });
  }
  ...
  if (body.action === 'delete') {
    ...
    persistServiceState('events.delete', { eventId: event.id });
  ...
  return json({ error: 'Unsupported action' }, 400);
    ...
    return json({ error: 'Internal error while saving event' }, 500);
  }
}
