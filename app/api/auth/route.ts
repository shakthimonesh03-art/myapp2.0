...
import { clearPhoneOtp, saveOtpForPhone, verifyPhoneOtp } from '@/lib/otpSql';
...
export async function POST(req: Request) {
  try {
    ...
    if (body.action === 'request-otp') {
      ...
      if (!sns.sent) {
        clearPhoneOtp(phone);
        const reason = sns.code === 'AuthorizationError'
          ? 'SNS IAM policy issue: current AWS user is not allowed to publish SMS.'
          : 'OTP delivery failed. Please verify SNS credentials, SMS settings, and phone number format.';
        return json({
          error: reason,
          troubleshooting: [
          ...
          snsError: sns.error || 'Unknown SNS error',
          snsErrorCode: sns.code || 'SNSPublishError'
        }, 502);
      }
