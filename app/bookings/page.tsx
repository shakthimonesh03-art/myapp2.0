...
export default function MyBookingsPage() {
  ...
  const upcomingBookings = bookings.filter((booking) => booking.status === 'CONFIRMED' && booking.cancellable);
  const allOtherBookings = bookings.filter((booking) => !(booking.status === 'CONFIRMED' && booking.cancellable));
    ...
    <section className="bookings-dashboard">
      ...
      {!isLoggedIn ? (
        <article className="booking-card">
          ...
          <a className="btn booking-cta" href="/auth">Go to Login</a>
        </article>
      ) : bookings.length === 0 ? (
        ...
        <div className="stack">
          <div>
            <h2 className="bookings-subtitle">Upcoming Bookings</h2>
            <div className="bookings-grid-3">
              {upcomingBookings.length === 0 ? (
                <article className="booking-card"><p>No upcoming bookings.</p></article>
              ) : upcomingBookings.map((booking) => {
                const isConfirmed = booking.status === 'CONFIRMED';
                return (
                  <article key={`up-${booking.id}`} className="booking-card">
                    <h3>{booking.eventTitle}</h3>
                    <p>Status: <span className={isConfirmed ? 'status-confirmed' : 'status-cancelled'}>{booking.status}</span></p>
          ...
          <div>
            <h2 className="bookings-subtitle">All Bookings</h2>
            <div className="bookings-grid-3">
              {allOtherBookings.length === 0 ? (
                <article className="booking-card"><p>No additional bookings.</p></article>
              ) : allOtherBookings.map((booking) => {
                const isConfirmed = booking.status === 'CONFIRMED';
                return (
                  <article key={`all-${booking.id}`} className="booking-card">
                    <h3>{booking.eventTitle}</h3>
                    <p>Status: <span className={isConfirmed ? 'status-confirmed' : 'status-cancelled'}>{booking.status}</span></p>
                    <p>Seats: {booking.seats.join(', ')}</p>
                    <p>Amount: ₹{booking.amount}</p>
