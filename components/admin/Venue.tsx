  ...
  return (
    <section className="admin-section stack">
      ...
      <article className="admin-card stack">
        ...
        <p className="muted">Step 1: choose event. Step 2: set rows/columns and prices. Step 3: click save.</p>
        <form className="admin-form seat-layout-form" onSubmit={saveLayout}>
          <label className="stack"><span>Event</span><select
            className="input"
          ...
          <label className="stack"><span>Rows</span><input className="input" type="number" min={1} value={layoutForm.rows} onChange={(e) => setLayoutForm({ ...layoutForm, rows: e.target.value })} /></label>
          <label className="stack"><span>Columns</span><input className="input" type="number" min={1} value={layoutForm.cols} onChange={(e) => setLayoutForm({ ...layoutForm, cols: e.target.value })} /></label>
          <label className="stack"><span>VIP rows</span><input className="input" type="number" min={0} value={layoutForm.vipRows} onChange={(e) => setLayoutForm({ ...layoutForm, vipRows: e.target.value })} /></label>
          <label className="stack"><span>Regular price</span><input className="input" type="number" min={1} value={layoutForm.regularPrice} onChange={(e) => setLayoutForm({ ...layoutForm, regularPrice: e.target.value })} /></label>
          <label className="stack"><span>VIP price</span><input className="input" type="number" min={1} value={layoutForm.vipPrice} onChange={(e) => setLayoutForm({ ...layoutForm, vipPrice: e.target.value })} /></label>
