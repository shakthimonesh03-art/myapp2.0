...
async function getSqsClient() {
  ...
  return new mod.SQSClient({
    ...
    credentials: process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY
      ...
      : undefined
  }) as { send: (cmd: unknown) => Promise<{ Messages?: { Body?: string; ReceiptHandle?: string }[] }> };
}
...
export async function publishSnsNotification(payload: { type: NotificationType; userId: string; message: string; timestamp: string; extra?: Record<string, unknown> }) {
  ...
  if (!topicArn) {
    console.warn('[aws] SNS_TOPIC_ARN not configured');
  ...
  try {
    ...
    const queueResult = await pushNotificationToQueue(payload);
    return { sent: true, queued: queueResult.queued };
  } catch (error) {
...
export async function sendOtpSms(phoneNumber: string, otpCode: string) {
  try {
    ...
    return { sent: false, error: err.message, code: err.name || 'SNSPublishError' };
  }
}
...
async function pushNotificationToQueue(payload: Record<string, unknown>) {
  return pushOtpAuditToQueue({ kind: 'notification', ...payload });
}

export async function consumeSqsMessages(handler: (body: string) => Promise<void>, retries = 3) {
  const queueUrl = process.env.SQS_QUEUE_URL;
  if (!queueUrl) {
    console.warn('[aws] SQS_QUEUE_URL not configured');
    return;
  }
