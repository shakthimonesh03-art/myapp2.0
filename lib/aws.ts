...
async function getSnsClient() {
  ...
  return new mod.SNSClient({ region: process.env.AWS_REGION || 'us-east-1' }) as { send: (cmd: unknown) => Promise<unknown> };
}

async function getSqsClient() {
  const mod = await runtimeImport<{ SQSClient: new (config: unknown) => unknown }>('@aws-sdk/client-sqs');
  return new mod.SQSClient({ region: process.env.AWS_REGION || 'us-east-1' }) as { send: (cmd: unknown) => Promise<{ Messages?: { Body?: string; ReceiptHandle?: string }[] }> };
}
