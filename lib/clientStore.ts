...
export function registerUser(name: string, email: string, password: string): { user?: AppUser; error?: string } {
  ...
  const nextUser: AppUser = { id: `U-${Date.now()}`, name: name.trim(), email: cleanEmail, password, role: 'customer' };
