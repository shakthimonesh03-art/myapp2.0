...
import { loadDbState, logOperation, saveDbState } from '@/lib/db';
...
const defaultState = {
  ...
  users: [] as { id: string; name: string; email: string; phone: string; role: string; cityPreference: string; passwordHash: string; createdAt: number; otpVerifiedAt?: number }[],
  events: [
    {
      id: 'ev1',
...
export const serviceState = loadDbState(defaultState);
...
export function persistServiceState(scope: string, meta?: Record<string, unknown>) {
  saveDbState(serviceState as unknown as Record<string, unknown>);
  logOperation(scope, 'state persisted', meta);
}
